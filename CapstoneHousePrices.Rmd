...
The analysis will start with the simplest model where the predicted Sale Price is the average of all given Sale Prices. This will produce a maximum RMSE that can be used as the initial reference point to beat.
...
See the appendix for some background on the XGBoost process

(see appendix [$\color{blue}{\text{(XGBoost)}}$](#xgb) for background to the XGBoost process).
...
## Final Validation
`r text_spec("Motivation",  color = "#d2691e", font_size= 10)`     
As can be seen the XGBoost method gave the lowest RMSE therefore this is the model that was chosen.
